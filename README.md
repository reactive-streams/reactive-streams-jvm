...
| <a name="2.6">6</a>       | A `Subscriber` MUST call `Subscription.cancel()` if it is no longer valid to its `Publisher`, without the `Publisher` having signaled `onError` or `onComplete`. |
